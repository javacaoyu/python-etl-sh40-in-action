...
from model.orders_model import OrdersDetailModel
from model.orders_model import OrdersModel

class TestOrdersDetailModel(TestCase):
    def test_generate_insert_sql_and_csv(self):
        ...
        self.assertEqual(excepted, model.to_cvs())

    def test_get_csv_header(self):
        excepted = "orderID,barcode,count,name,unitID,pricePer,retailPrice,tradePrice,categoryID"
        self.assertEqual(excepted, OrdersDetailModel.get_csv_header())

    def test_generate_insert_sql_and_csv_another(self):
        data = '{"discountRate": 1, "storeShopNo": "None", "dayOrderSeq": 37, "storeDistrict": "芙蓉区", ' \
               '"isSigned": 0, "storeProvince": "湖南省", "origin": 0, "storeGPSLongitude": "undefined", ' \
               '"discount": 0, "storeID": 1766, "productCount": 1, "operatorName": "OperatorName", ' \
               '"operator": "NameStr", "storeStatus": "open", "storeOwnUserTel": 12345678910, ' \
        ...
        excepted = "154243648991517662217,6901028300056,1,南京特醇,8,12,12,11,1"
        self.assertEqual(excepted, orders_detail_model.to_cvs())