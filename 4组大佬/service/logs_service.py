...
from util import file_util, time_util, logging_util
...
class LogsDataService:
    ...
    def __init__(self):
        ...
        self.logs_csv_path = project_config.csv_output_root_path + \
    ...
    def __write(self, models_list):
        ...
        # csv改名
        file_util.change_file_suffix(self.logs_csv_path, target_suffix="", origin_suffix=".tmp")


    def __write_to_csv(self, models_list):

        # 获取写文件对象
        logs_csv_writer = open(self.logs_csv_path, 'w', encoding="UTF-8")


        # 写出csv标头
        logs_csv_writer.write(LogsDataModel.get_csv_header())
        logs_csv_writer.write("\n")


        logs_counter = 0
        for model in models_list:
            csv_line = model.to_csv()           # 订单 to csv
            logs_csv_writer.write(csv_line)
            logs_csv_writer.write("\n")
            logs_counter += 1

        logs_csv_writer.close()
        self.logger.info(f"【logs订单数据采集】写出数据到CSV文件：{self.logs_csv_path}, 条数：{len(models_list)}")
    ...
    def __write_to_mysql(self, models_list):
        ...
        self.target_mysql_util.check_and_create_table(
            ...
            table_name=db_config.target_logs_data_table_name,
        ...
        logs_counter = 0
        for model in models_list:
            sql = model.get_insert_sql()       # 表的插入语句
            self.target_mysql_util.execute(db_config.target_db_name, sql)
            logs_counter += 1

        self.logger.info(f"【logs数据采集】本次即将向数据库提交数据{len(models_list)}条。")

    def get_models_list(self, files):
        """
        读取文件，转换成logs模型，封装到list内
        :param files: list记录文件路径
        :return: list（内函LogsDataModel对象）
        """
        models_list = []
        for path in files:
    ...
    def get_need_to_process_file_list(
            self,
            files_dir=project_config.logs_file_data_path,
            db=db_config.metadata_db_name,
